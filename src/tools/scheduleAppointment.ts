  ...
  return tool({
    ...
    execute: async ({ date, startTime, attendeeName, attendeeEmail, service, notes }) => {
      if (!calcomConfigured(env)) {
        // SIN agenda en línea: guía explícita para que el modelo NO alucine
        // reservas ni escale: captura el lead y avisa que el negocio confirmará.
        return {
          error: "booking_unavailable",
          guidance:
            "No hay agenda en línea configurada. NO digas que agendaste ni confirmaste nada. " +
            "Captura la solicitud con captureLead (intent = la cita pedida, name, contact, metadata { fecha, hora } si aplica) " +
            "y dile al cliente que el negocio le confirmará por mensaje.",
        } as const;
      }
