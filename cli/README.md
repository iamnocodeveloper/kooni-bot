# forjabot (CLI del upstream — NO usar en Kooni)

> ⚠️ **Aviso (repo Kooni):** esta carpeta `cli/` es el CLI original de **Forja**
> (upstream, MIT). Está incluida solo como referencia del proyecto fuente. En Kooni
> **NO se usa**: apunta a servidores de licencias de Horizontes IA y su uso no aporta
> nada al despliegue interno. Despliega directo con `git clone` + `pnpm run deploy`
> (ver `docs/DESPLIEGUE.md`). Todo lo que sigue es documentación del CLI original.
