...
| `D1 create ... already exists` | la base de datos ya existía | corre `pnpm wrangler d1 list`, copia el `database_id` real y pégalo en `wrangler.toml` (binding **DB**, `kooni_db`) |
| `Vectorize ... already exists` | el índice ya existía | corre `pnpm wrangler vectorize list` y reutiliza `kooni_kb` (no lo vuelvas a crear) |
...
```bash
pnpm wrangler d1 create kooni_db
pnpm wrangler vectorize create kooni_kb --dimensions=1024 --metric=cosine
```
...
| `Vectorize: index not found` | el índice no existe | `pnpm wrangler vectorize create kooni_kb --dimensions=1024 --metric=cosine` |
