# Privacidad y datos — Kooni

Kooni es software **self-hosted**: se instala en **tu propia cuenta de Cloudflare**, con **tus propias llaves**. Ninguna otra persona recibe, ve o guarda las conversaciones de tus clientes.
...
Cuando instalas Kooni, **tú eres el responsable** de los datos personales de tus clientes; Kooni es solo la herramienta. Con eso en mente:
...
## 5. Panel externo (opcional, desactivado)

Existe una API opcional en `/api/*` para conectar el bot a un panel externo. Está **apagada por defecto**: solo responde si configuras el secreto `CONTROL_PLANE_TOKEN`, y aun activada devuelve **únicamente números agregados** (leads, mensajes, conversaciones) más la versión del bot. Nunca el contenido de una conversación ni datos de una persona.

---

Kooni se entrega **tal cual**, bajo licencia MIT y sin garantías (ver [LICENSE](./LICENSE)). El cumplimiento legal de tu operación es responsabilidad tuya.
