...
describe("scheduleAppointmentTool", () => {
  ...
  it("sin Cal.com devuelve guía accionable (NO alucinar reservas → captureLead)", async () => {
    const tool = scheduleAppointmentTool({ BOT_TIER: "pro" } as any, () => "conv_x");
    const result = (await tool.execute!(
      { date: "2027-06-01", startTime: "2027-06-01T17:00:00-06:00" },
      {} as any,
    )) as { error: string; guidance: string };
    expect(result.error).toBe("booking_unavailable");
    expect(result.guidance).toContain("captureLead");
    expect(result.guidance).toContain("NO digas que agendaste");
  });
  ...
  it("returns booking_unavailable (guía) when no API key", async () => {
    global.fetch = vi.fn() as any;
  ...
  it("returns booking_unavailable (guía) when no event type is configured", async () => {
    global.fetch = vi.fn() as any;
