...
beforeEach(async () => {
  ...
  env = {
    ...
    BOT_TIER: "pro",
    BUFFER_SECONDS: "12",
    // Con Cal.com configurado, la nota "no agenda en línea" NO se inyecta.
    CALCOM_API_KEY: "cal_test",
    CALCOM_EVENT_TYPE_ID: "100",
  };
...
describe("resolveAgentConfig", () => {
  ...
  it("parses paused_channels as a JSON array (canales pausados individualmente)", async () => {
    expect((await resolveAgentConfig(env, TOOLS)).pausedChannels).toEqual([]);
    await repo.set(SETTING_KEYS.pausedChannels, JSON.stringify(["telegram", "zernio"]));
    expect((await resolveAgentConfig(env, TOOLS)).pausedChannels).toEqual(["telegram", "zernio"]);
    // JSON inválido => lista vacía (nunca truena)
    await repo.set(SETTING_KEYS.pausedChannels, "not-json");
    expect((await resolveAgentConfig(env, TOOLS)).pausedChannels).toEqual([]);
    await repo.set(SETTING_KEYS.pausedChannels, "");
  });
});
...
describe("resolveAgentConfig — custom_instructions", () => {
  ...
  it("sin Cal.com inyecta la nota anti-alucinación de reservas", async () => {
    const envNoCal = { ...env, CALCOM_API_KEY: undefined, CALCOM_EVENT_TYPE_ID: undefined };
    const cfg = await resolveAgentConfig(envNoCal, TOOLS);
    expect(cfg.systemPrompt).toContain("No hay agenda en línea configurada");
    expect(cfg.systemPrompt).toContain("captureLead");
    expect(cfg.systemPrompt).toContain("NUNCA digas que agendaste");
  });
