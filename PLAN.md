...
19. **Registro de auditoría del panel** (`§ U`) — 🚧 **U1 + U2 HECHOS (2026-09-03), pendiente commit+release.** Ventana `/admin/auditoria` de **solo lectura** (Pro): quién entró, hora, qué acción, qué modificó, **antes → después**. Decisiones de Joel: **Nivel A** (huella IP + navegador, sin tocar el login) · **vista solo Pro**, captura siempre. U1: tabla `audit_log`, `AuditRepo`, `src/audit/context.ts` (`AsyncLocalStorage` + redacción de secretos), middleware de actor, `SettingsRepo.set` instrumentado, captura de login/logout, purga nocturna (180 d). U2: la ventana (`src/admin/views/auditoria.ts`), filtros, `export.csv`, nav con candado, módulo vendible. Falta: **U3** `recordAudit()` en las ~12 rutas fuera de Settings (KB, automatizaciones, leads, tickets, conversaciones, mejoras, push). Detalle en `§ U`.
