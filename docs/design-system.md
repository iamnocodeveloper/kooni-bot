...
|---|---|---|---|
| `--bg` | `bg-bg` | `#0d1218` | page background (already on `<body>`) |
| `--panel` | `bg-panel` | `#141b24` | card / panel surface |
| `--panel2` | `bg-panel2` | `#1a2330` | nested surface, row hover, inputs-on-panel |
| `--raise` | `bg-raise` | `#223043` | raised chips / avatars |
| `--line` | `border-line` | `#2b3b4f` | default border / divider |
| `--linelit` | `border-linelit` | `#3d546f` | lit border, hard-shadow color |
| `--accent` | `text-accent` `bg-accent` `border-accent` | `#2dd4bf` | primary accent (teal) |
| `--accent-soft` | `bg-accent-soft` | `rgba(45,212,191,.13)` | accent wash / active bg |
| `--accent-2` | `text-accent2` | `#6ee7b7` | secondary accent (mint): AI/insights |
| `--cream` | `text-cream` | `#e7eef5` | primary text |
| `--muted` | `text-muted` | `#93a4b5` | secondary text |
| `--dim` | `text-dim` | `#64748b` | tertiary text, labels, captions |
| `--ok` | `text-ok` `border-ok` | `#34d399` | success / green (resolved, online) |
| `--info` | `text-info` `border-info` | `#60a5fa` | info / blue (WhatsApp, escalated) |
| `--bad` | `text-bad` `border-bad` | `#f87171` | danger / red (angry, handoff, errors) |
| `--violet` | `text-violet` | `#a78bfa` | model/memory accents in the flow canvas |

Buttons on `--accent` use text color `#06251f` (dark ink on teal) — there is
